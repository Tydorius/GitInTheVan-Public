...
The purpose of this script is to provide detailed output and object definitions for the available properties on the GitInTheVan Scripts `context` object. The context auto-complete was used to identify everything available when Scripts first came out, and a wrapper was put on everything to help identify each property by type while also providing example data. You can run this and look at the debug console to see how data is formatted.
